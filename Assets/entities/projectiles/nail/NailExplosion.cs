...
public class NailExplosion : MonoBehaviour
{
    ...
    void Explode()
    {
        ...
        for(int i = 0; i < hitColliders.Length; i++)
        {
            ...
            else if (hitColliders[i].gameObject.GetComponent<IDamageable>() != null)
            {
                ...
                {
                    hitColliders[i].gameObject.GetComponent<Boid>().velocity += Vector3.Normalize(hitColliders[i].transform.position - transform.position) * forceConstant;
                }
                else
                {
                    ...
                    Vector3 forceDir = Vector3.Normalize(transform.forward);
                    //enemy.gObj.GetComponent<Rigidbody>().AddForce(forceDir * forceMult * forceConstant, ForceMode.VelocityChange);
                }
            }
        }
    }
